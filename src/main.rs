...
use rand::Rng;
    ...
    impl<Message> Widget<Message, Renderer> for Circle {
        ...
        let mut prims = vec![];
                ...
                for i in 0..101 {
			let mut frame = Frame::new(layout.bounds().width, layout.bounds().height);
// 			frame.translate(iced::Vector::new(rand::thread_rng().gen_range(0.0, 600.0), rand::thread_rng().gen_range(0.0, 600.0)));
			frame.stroke(&Path::line([100., 100.].into(), [200.,200.,].into()), Stroke {
					width: 2.0,
					color: iced::Color::BLACK,
					..iced::canvas::Stroke::default()
				});
 			prims.push(frame.into_primitive());
                };
            (
